...
3. **Component Repository** — реестр компонентов, версии, стабильность, зависимые пакеты.
...
## MVP → v1 (расширенный план)
1. **MVP**: базовый редактор, сценарный рантайм, ядро компонентов.
2. **v0.9**: шаблоны, модель данных, автотесты, библиотека компонентов.
3. **v1.0**: публикация на все платформы, каталог и marketplace.
